...
#import "libs/presets/thesis.typ": heading-token-params, page-settings, style-tokens as s
...
#thesis-cover(
  ...
  author: "著者氏名",
  title-styler: style.with((s.text-4xl, s.font-sans, s.font-bold)),
  subtitle-styler: style.with((s.text-2xl,)),
  author-styler: style.with((s.text-2xl,)),
  small-styler: style.with((s.text-xl,)),
)
...
#thesis-cover(
  ...
  author-styler: style.with((s.text-no-justify, s.text-2xl)),
  small-styler: style.with((s.text-no-justify, s.text-xl)),
)

// 論文要旨、目次

#thesis-abstract(
  (lang: "ja", heading: [論文要旨], body: roremu(800)),
  (lang: "en", heading: [Abstract], body: lorem(300)),
)
...
#thesis-outline(
  ...
  strong-styler: style.with((
    s.font-sans,
    s.font-semibold,
    (block-above: s.text-base.line-spacing * 2 + s.text-base.text-size),
  )),
)
