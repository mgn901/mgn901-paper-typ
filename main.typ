...
#article-frontmatter(
  ...
  author-styler: style.with((s.text-lg,)),
...
#figure(
  ...
  table(
    ...
    [参照したい項目に番号をつけると，その項目を参照可能になる．参照の挿は，ダイアログに表示される参照可能な項目の一覧から選んで行う．項目の増減があった場合は「フィールドの更新」操作を行う必要がある．これらの操作はマウスで画面を操作することで行う．],
    ...
    [「テンプレート」を作成，編集する．組み込みの要素をどのように組版するのかを，別の組み込みの要素を用いて表現する「ルール」を組み合わせてテンプレートを作る．],
    [2024年時点で和文組版の機能が#[#LaTeX]の資産に比べて貧弱であるが，バージョンアップを重ねるごとに改善が進んでいる．組版の速さは速い．],
  ),
